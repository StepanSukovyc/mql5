...
Hybrid vyhodnocuje pouze ztrátové pozice otevřené od `HYBRID_EXIT_ANALYSIS_START_DATE`; starší pozice jsou vždy `legacy_excluded`. Stáří je počítáno v aktivních obchodních hodinách s výjimkou konfigurovaného FX víkendu. Stará pozice mimo break-even buffer může být uzavřena až při alespoň dvou negativních reason codes z uzavřených H1/H4 svíček. Výchozí `HYBRID_EXIT_DRY_RUN=true` jen loguje rozhodnutí.
